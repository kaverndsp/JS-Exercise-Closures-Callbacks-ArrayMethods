...
function processContains(item, list, cb) {
 const value = list.includes(item);
 return cb(value);
}
...
function getFullNames(list) {
  const newList = [];
  list.forEach(function(runner){
    newList.push(`${runner.last_name}, ${runner.first_name}`);
...
function firstNamesAllCaps(list) {
  const newList = list.map(function(name){
...
function tallyUpDonations(runners) {
  const newArray = [];
  runners.reduce(function(total, donations){
    newArray.push(total + donations.donation);
